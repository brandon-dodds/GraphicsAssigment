...
{	//LIGHTING
    ...
    pixel = texture(aTex,uv).rgb;
    
    float t = sin(time/1000.0);
    float erosion = smoothstep(t-.2, t, noise);
    
    float border = smoothstep(0., .1, erosion) - smoothstep(.1, 1., erosion);

    pixel = pixel * (1. - erosion) + erosion * texture(aTex2, uv).rgb;
    
    pixel = pixel + border * vec3(0., 1., 0.);
	
	//vertColour = vec4(temp);
	//vertColour = texture(aTex2, uv);
	vertColour = vec4((ambient+diffuse+specular),1.0) * vec4(pixel, 1.);

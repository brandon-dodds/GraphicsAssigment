...
int main(int argc, char *argv[]) {
	...
	//once only scale to cube
	cubeScale = glm::scale(cubeScale, glm::vec3(0.5f, 0.5f, 0.5f));
	errorLabel = 4;
	...
	while (windowOpen)
	{
		...
		//rotation
		cubeRotate = glm::rotate(cubeRotate, (float)elapsedTime / 1000, glm::vec3(0.0f, 1.0f, 0.0f));
		cubeModelLocation = glGetUniformLocation(cube.shaderProgram, "uModel");
		glUniformMatrix4fv(cubeModelLocation, 1, GL_FALSE, glm::value_ptr(cubeRotate*cubeScale));
